...
description: "Produce a rigorous, stress-tested research essay delivered as a downloadable PDF. Use when the user asks for a report/essay on a subject (e.g. what humans cannot see/hear vs. what devices detect — multispectral, hyperspectral, infrared, UV, radar, sonar, ultrasound), a business/profit angle on such a topic, or a stress test of a previous answer of this type. Clean academic layout (Calibri/Carlito 12pt, double-spaced, numbered section titles, author + date only — no course/instructor header), a plain centered page number on each page (no name in the header), every claim anchored to a measurable quantity or named source, at least three verbatim attributed quotes, a risk/falsification block, and exactly two Questions for Further Research. Token-light baseline (~3,000–5,000 tokens) that scales depth up when more source data is supplied."
...
Produce a rigorous, stress-tested research essay, delivered as a downloadable PDF.
...
- Use a clean academic look for any essay/report-length output:
  ...
  - Page numbers: number every page with a plain page number only (e.g. centered in the footer). Do NOT print any name, surname, or other personal identifier in the header or footer.
...
### 2. No watermark or name markings

- Do NOT apply any watermark, tiled diagonal stamp, or background name overlay to the page.
- Do NOT print the author's name (or any personal identifier) in the running header, footer, or as a watermark. The only place a name may appear is the single author line at the top of page 1.
...
| 4 | Early outputs had no watermark vs. later "add watermark" vs. latest "remove the watermark, no name on each page." | No watermark and no name in the header/footer. Number every page with a plain page number only. |
...
- [ ] NO watermark anywhere; NO name in the header/footer; pages numbered with a plain page number only.
...
- "Make a report on what cameras and mics detect beyond human senses." → Full PDF, Standard Question Set, three+ quotes, two questions; no watermark, plain page numbers only.
- "Stress test it." → Re-run the stress-test procedure (Rule 3), add one new stress idea, regenerate the PDF, attach.
...
- Adding a watermark, or printing a name in the header/footer of any page.
